...
class srcList:
	...
	def makeModel(self,GDfile="$(FERMI_DIR)/refdata/fermi/galdiffuse/gll_iem_v07.fits",GDname='gll_iem_v07',ISOfile="$(FERMI_DIR)/refdata/fermi/galdiffuse/iso_P8R3_SOURCE_V2_v1.txt",ISOname='iso_P8R3_SOURCE_V2_v1',normsOnly=False,extDir='',radLim=-1,maxRad=None,ExtraRad=10,sigFree=5,varFree=True,psForce=False,E2CAT=False,makeRegion=True,GIndexFree=False,wd='',oldNames=False):
...
print "This is make4FGLxml version 01r04."
...
d2r=pi/180.
varValue=72.44#need to update this when 4FGL is actually ready
...
def addSrcsFITS(sL,GD,GDn,ISO,ISOn,oldNames):
	...
	for x in radii:
		...
		for n,plf,lpf,cof,r,d,p,pli,lpi,lpb,pleci,plecef,plecei,t,TS,En in zip(name,plflux,lpflux,coflux,ra,dec,pivot,plIndex,lpIndex,lpbeta,plecIndex,plecexpFact,plecexpIndex,spectype,Sigvals,EName):
...
def CO2spec(sL,f,i,p,a,ei,dist,TS,vi):
	...
	if(dist>sL.roi[2]): #if beyond ROI, shouldn't attempt to fit parameters
		...
		if dist>sL.maxRad:
			...
			free=True
		spec+='\t\t<parameter free="0" max="10.0" min="0.0" name="Index1" scale="-1.0" value="%s"/>\n' %i
		spec+='\t\t<parameter free="0" max="100" min="0.1" name="Expfactor" scale="0.001" value="%f"/>\n'%(a*1000.)
	elif(TS<sL.sig):
		if vi<varValue or not sL.var:
			spec+='\t<!-- Source significance %.1f is less than specified minimum for a free source of %s -->\n'%(TS,sL.sig)
			spec+='\t\t<parameter free="0" max="1e4" min="1e-4" name="Prefactor" scale="1e%i" value="%s"/>\n' %(fscale,f/10**fscale)
		...
		if sL.nO:
			...
			spec+='\t\t<parameter free="1" max="10.0" min="0.0" name="Index1" scale="-1.0" value="%s"/>\n' %i
			spec+='\t\t<parameter free="1" max="100" min="0.1" name="Expfactor" scale="0.001" value="%f"/>\n'%(a*1000.)
	spec+='\t\t<parameter free="0" max="5e5" min="30" name="Scale" scale="1.0" value="%f"/>\n'%p
	spec+='\t\t<parameter free="0" max="5" min="0" name="Index2" scale="1.0" value="%f"/>\n'%ei
	spec+='\t</spectrum>\n'
	return spec,free
...
def cli():
	...
	parser.add_argument("-I","--isofile",type=str,default='$(FERMI_DIR)/refdata/fermi/galdiffuse/iso_P8R3_SOURCE_V2_v1.txt',help="Name of isotropic diffuse template for output model, will default to P8R3 SOURCE class model.")
